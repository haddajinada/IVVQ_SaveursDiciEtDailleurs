...
<div class="fieldcontain ${hasErrors(bean: defiInstance, field: 'categorie', 'error')} required">
	...
	<g:select id="categorie" name="categorie.id" from="${ivvq_saveursdicietdailleurs.Categorie.list()}" optionKey="id" required="" value="${defiInstance?.categorie?.nomCategorie}" class="many-to-one"/>
</div>
